...
function mzk_add_video($post_id, $meta_key, $label){
    ...
    <?php if($meta_value && $meta_value['url'] && $meta_value['file']):?>
      <div id="<?php echo $meta_key ?>_show" style="<?php if(!$meta_value) echo 'display: none';?>">
        ...
        <div><a href="#" id="remove_video" 
          data-postid="<?php echo $post_id;?>" 
          data-value="<?php echo $meta_value['file']?>" 
          data-type="<?php echo $meta_value['type']?>"
          data-key = "<?php echo $meta_key ?>"
          class="remove_video"
          target="_blank">Remove file</a></div>
      </div>
    <?php endif;?>
  ...
  if (wp_doing_ajax()) {
    add_action('wp_ajax_remove_video', 'remove_video_callback');
    function remove_video_callback()
    {
      if (
